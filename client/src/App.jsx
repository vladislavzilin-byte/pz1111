...
export default function App() {
  ...
  // --- Email verification (server-based) ---
  async function sendVerificationCode(email) {
    try {
      ...
      if (data.ok) { alert(`Код отправлен на ${email}. Проверьте почту.`); return true; }
    } catch (e) {}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white p-4 rounded max-w-sm w-full">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={async()=>{ if(!loginEmail){ alert('Введите email'); return; } const ok = await sendVerificationCode(loginEmail); if(ok){ setOpenVerify(true); } }} className="px-3 py-1 bg-blue-600 text-white rounded">Отправить код</button>
            </div>
            <div className="text-xs text-gray-500 mt-2">Код отправляется на ваш email. Если письма нет — проверьте «Спам».</div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white p-4 rounded max-w-sm w-full">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={async()=>{ const code = (document.getElementById('verifCodeInput')?.value||'').replace(/\D/g,''); const ok = await verifyCode(loginEmail, code); if(ok){ setOpenVerify(false); setOpenLogin(false); } else { alert('Неверный код'); } }} className="px-3 py-1 bg-green-600 text-white rounded">Подтвердить</button>
            </div>
          </div>
        </div>
      )}
